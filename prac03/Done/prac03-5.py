...
# --------------------------
# 플레이어 캐릭터 생성
player = Creature("Hero", 50, 5, [Skill("Slash", 4, 95), Skill("Fire", 6, 85)])

# 적 캐릭터 생성
enemies = [
    Creature("SlimeA", 30, 3, [Skill("Bite", 3, 90)]),
    Creature("SlimeB", 25, 3, [Skill("Bite", 3, 90)]),
    Creature("SlimeC", 20, 2, [Skill("Bite", 2, 95)])
]
...
while running:
            ...
            # ----------------------
                ...
                for i, skill in enumerate(player.skills):
                    ...
                    if mouse_pos[0] > button.x and mouse_pos[0] < button.x+button.width:
                        if mouse_pos[1] > button.y and mouse_pos[1] < button.y+button.height:
                            choice = i
                back_button = pygame.Rect(screen_width//2-button_width//2, 400+(i+1)*40-button_height//2, button_width, button_height)
                ...
                for i, enemy in enumerate(enemies):
                    ...
                    if mouse_pos[0] > x and mouse_pos[0] < x+80 and mouse_pos[1] > y and mouse_pos[1] < y+80:
                        ...
                        for enemy in enemies:
                            if enemy.alive():                                   # 살아있는 적이 공격
                                log.append(attack(enemy, player, random.choice(enemy.skills)))
    ...
    if not gameover:
        if menu == 0:
            ...
            for i, skill in enumerate(player.skills):
                draw_text(screen, font, skill.name, (screen_width//2, 400+i*40), (0,200,200) if i==choice else colors["WHITE"], True)
            draw_text(screen, font, "Back", (screen_width//2, 400+(i+1)*40), colors["WHITE"], True)
